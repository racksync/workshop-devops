...
เพื่อให้ GitHub Actions runner สามารถรันคำสั่งที่ต้องใช้สิทธิ์ root โดยไม่ต้องถามรหัสผ่านทุกครั้ง เราจำเป็นต้องแก้ไขไฟล์ sudoers โดยใช้คำสั่ง `visudo` ดังนี้:

1. เปิดไฟล์ sudoers ด้วยคำสั่ง `visudo`:
   ```bash
   sudo visudo
   ```
...
2. เพิ่มบรรทัดต่อไปนี้ที่ส่วนท้ายของไฟล์:
   ...
   **คำอธิบาย:**
   - `runner` คือชื่อผู้ใช้ของ runner ที่เราต้องการให้สามารถใช้ sudo โดยไม่ต้องถามรหัสผ่าน
   - `ALL=(ALL) NOPASSWD:ALL` หมายความว่า ผู้ใช้ `runner` สามารถรันคำสั่งใด ๆ ด้วย sudo โดยไม่ต้องใส่รหัสผ่าน

3. บันทึกและปิดไฟล์ sudoers

**ผลกระทบต่อ GitHub Actions:**

การตั้งค่านี้จะช่วยให้ GitHub Actions runner สามารถรันคำสั่งที่ต้องใช้สิทธิ์ root ได้โดยไม่ต้องถามรหัสผ่านทุกครั้ง ซึ่งจะช่วยให้ workflow ของคุณทำงานได้อย่างราบรื่นและไม่ติดขัดในขั้นตอนที่ต้องใช้ sudo เช่น การติดตั้งแพ็คเกจหรือการจัดการไฟล์ระบบ
...
ในตัวอย่างนี้ คำสั่ง `sudo apt-get update` และ `sudo apt-get install` จะรันโดยไม่ต้องถามรหัสผ่าน เนื่องจากเราได้ตั้งค่าให้ผู้ใช้ runner สามารถใช้ sudo โดยไม่ต้องใส่รหัสผ่านแล้ว
